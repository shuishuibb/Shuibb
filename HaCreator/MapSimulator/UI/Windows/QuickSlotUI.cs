...
namespace HaCreator.MapSimulator.UI
{
    ...
    public class QuickSlotUI : UIWindowBase
    {
        ...
        private readonly Dictionary<int, Texture2D> _npcPreviewTextureCache = new();
        ...
        private void DrawHoveredItemTooltip(SpriteBatch sprite, int renderWidth, int renderHeight, int absoluteSlotIndex)
        {
            ...
            Texture2D infoEffectPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                ? null
                : ResolveInfoEffectPreviewTexture(itemId);
            Texture2D infoPathPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                                                                 || infoEffectPreviewTexture != null
                ? null
                : ResolveInfoPathPreviewTexture(itemId);
            Texture2D rootEffectPreviewTexture = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures)
                                                                      || infoEffectPreviewTexture != null
                                                                      || infoPathPreviewTexture != null
                ? null
                : ResolveRootEffectPreviewTexture(itemId);
            Texture2D[] mobPreviewTextures = HasDrawableBitmap(iconRewardTextures) || HasDrawableBitmap(rewardPreviewTextures) || infoEffectPreviewTexture != null || infoPathPreviewTexture != null || rootEffectPreviewTexture != null
                ? Array.Empty<Texture2D>()
                : ResolveMobPreviewTextures(itemId);
            Texture2D[] consumeItemPreviewTextures = HasDrawableBitmap(iconRewardTextures)
                                                     || HasDrawableBitmap(rewardPreviewTextures)
                                                     || infoEffectPreviewTexture != null
                                                     || infoPathPreviewTexture != null
                                                     || rootEffectPreviewTexture != null
                                                     || HasDrawableBitmap(mobPreviewTextures)
                ? Array.Empty<Texture2D>()
                : ResolveConsumeItemPreviewTextures(itemId);
            Texture2D npcPreviewTexture = HasDrawableBitmap(iconRewardTextures)
                                          || HasDrawableBitmap(rewardPreviewTextures)
                                          || infoEffectPreviewTexture != null
                                          || infoPathPreviewTexture != null
                                          || rootEffectPreviewTexture != null
                                          || HasDrawableBitmap(mobPreviewTextures)
                                          || HasDrawableBitmap(consumeItemPreviewTextures)
                ? null
                : ResolveNpcPreviewTexture(itemId);
            ...
            int primaryIconWidth = Math.Max(SLOT_SIZE, primaryIconTexture?.Width ?? 0);
            int primaryIconHeight = Math.Max(SLOT_SIZE, primaryIconTexture?.Height ?? 0);
            ...
            float infoEffectPreviewHeight = infoEffectPreviewTexture?.Height ?? 0f;
            float infoPathPreviewHeight = infoPathPreviewTexture?.Height ?? 0f;
            float rootEffectPreviewHeight = rootEffectPreviewTexture?.Height ?? 0f;
            float mobPreviewHeight = MeasureHorizontalBitmapStripHeight(mobPreviewTextures);
            float consumeItemPreviewHeight = MeasureHorizontalBitmapStripHeight(consumeItemPreviewTextures);
            float npcPreviewHeight = npcPreviewTexture?.Height ?? 0f;
            ...
            else if (infoEffectPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TOOLTIP_BITMAP_GAP : 0f) + infoEffectPreviewHeight;
            }
            else if (infoPathPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TOOLTIP_BITMAP_GAP : 0f) + infoPathPreviewHeight;
            }
            ...
            else if (mobPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TOOLTIP_BITMAP_GAP : 0f) + mobPreviewHeight;
            }
            else if (consumeItemPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TOOLTIP_BITMAP_GAP : 0f) + consumeItemPreviewHeight;
            }
            else if (npcPreviewHeight > 0f)
            {
                bitmapPreviewHeight += (bitmapPreviewHeight > 0f ? TOOLTIP_BITMAP_GAP : 0f) + npcPreviewHeight;
            }
            ...
            if (primaryIconTexture != null)
            {
                ...
                {
                    sprite.Draw(primaryIconTexture, new Rectangle(backgroundRect.X + TOOLTIP_PADDING, contentY, SLOT_SIZE, SLOT_SIZE), Color.White);
                }
            }
            ...
            if (sampleTexture != null)
            {
                ...
                DrawCenteredTooltipBitmap(
                    sprite,
                    infoEffectPreviewTexture,
                    backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - (infoEffectPreviewTexture?.Width ?? 0)) / 2),
                    stripY,
                    infoEffectPreviewTexture?.Width ?? 0);
                DrawCenteredTooltipBitmap(
                    sprite,
                ...
                DrawCenteredTooltipBitmap(
                    sprite,
                    infoPathPreviewTexture,
                    backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - (infoPathPreviewTexture?.Width ?? 0)) / 2),
                    stripY,
                    infoPathPreviewTexture?.Width ?? 0);
                DrawHorizontalBitmapStrip(sprite, mobPreviewTextures, backgroundRect, stripY);
                DrawHorizontalBitmapStrip(sprite, consumeItemPreviewTextures, backgroundRect, stripY);
                DrawCenteredTooltipBitmap(
                    sprite,
                    npcPreviewTexture,
                    backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - (npcPreviewTexture?.Width ?? 0)) / 2),
                    stripY,
                    npcPreviewTexture?.Width ?? 0);
            }
            ...
            {
                int effectX = backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - infoEffectPreviewTexture.Width) / 2);
                sprite.Draw(infoEffectPreviewTexture, new Vector2(effectX, previewY), Color.White);
            }
            else if (infoPathPreviewHeight > 0f)
            {
                int effectX = backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - infoPathPreviewTexture.Width) / 2);
                sprite.Draw(infoPathPreviewTexture, new Vector2(effectX, previewY), Color.White);
            }
            else if (rootEffectPreviewHeight > 0f)
            {
            ...
            {
                DrawHorizontalBitmapStrip(sprite, mobPreviewTextures, backgroundRect, previewY);
            }
            else if (consumeItemPreviewHeight > 0f)
            {
                DrawHorizontalBitmapStrip(sprite, consumeItemPreviewTextures, backgroundRect, previewY);
            }
            else if (npcPreviewHeight > 0f)
            {
                int npcX = backgroundRect.X + Math.Max(TOOLTIP_PADDING, (backgroundRect.Width - npcPreviewTexture.Width) / 2);
                sprite.Draw(npcPreviewTexture, new Vector2(npcX, previewY), Color.White);
            }
        }
        ...
        private Texture2D ResolveInfoPathPreviewTexture(int itemId)
        {
            if (itemId <= 0 || _graphicsDevice == null)
            {
                return null;
            ...
            if (_infoPathPreviewTextureCache.TryGetValue(itemId, out Texture2D cachedTexture))
            {
                return cachedTexture;
            }

            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveInfoPathFirstCanvas(itemId, out WzCanvasProperty pathCanvas))
            {
                texture = pathCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(_graphicsDevice);
            }

            _infoPathPreviewTextureCache[itemId] = texture;
            return texture;
        }

        private Texture2D ResolveInfoEffectPreviewTexture(int itemId)
        {
            if (itemId <= 0 || _graphicsDevice == null)
            {
                return null;
            ...
            if (_infoEffectPreviewTextureCache.TryGetValue(itemId, out Texture2D cachedTexture))
            {
                return cachedTexture;
            }

            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveInfoEffectFirstCanvas(itemId, out WzCanvasProperty effectCanvas))
            {
                texture = effectCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(_graphicsDevice);
            }

            _infoEffectPreviewTextureCache[itemId] = texture;
            return texture;
        }

        private Texture2D[] ResolveMobPreviewTextures(int itemId)
        {
            if (itemId <= 0 || _graphicsDevice == null)
            {
                return Array.Empty<Texture2D>();
            ...
            if (_mobPreviewTextureCache.TryGetValue(itemId, out Texture2D[] cachedTextures))
            {
                return cachedTextures;
            }

            IReadOnlyList<InventoryMobPreviewItem> mobItems =
                InventoryItemMetadataResolver.ResolveMobPreviewItems(itemId, 8);
            List<Texture2D> textures = new(mobItems.Count);
            for (int i = 0; i < mobItems.Count; i++)
            {
                if (!InventoryItemMetadataResolver.TryResolveMobPreviewCanvas(
                        mobItems[i],
                        out WzCanvasProperty mobCanvas))
                {
                    continue;
                }

                Texture2D texture = mobCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(_graphicsDevice);
                if (texture != null)
                {
                    textures.Add(texture);
                }
            ...
            Texture2D[] resolvedTextures = textures.ToArray();
            _mobPreviewTextureCache[itemId] = resolvedTextures;
            return resolvedTextures;
        }

        private Texture2D[] ResolveConsumeItemPreviewTextures(int itemId)
        {
            if (itemId <= 0 || _graphicsDevice == null)
            {
                return Array.Empty<Texture2D>();
            ...
            if (_consumeItemPreviewTextureCache.TryGetValue(itemId, out Texture2D[] cachedTextures))
            {
                return cachedTextures;
            }

            IReadOnlyList<ConsumeItemRequirementMetadata> consumeItems =
                InventoryItemMetadataResolver.ResolveConsumeItemRequirements(itemId);
            List<Texture2D> textures = new(consumeItems.Count);
            for (int i = 0; i < consumeItems.Count; i++)
            {
                if (!InventoryItemMetadataResolver.TryResolveTooltipIconCanvas(
                        consumeItems[i].ItemId,
                        out WzCanvasProperty iconCanvas))
                {
                    continue;
                }
            ...
            Texture2D[] resolvedTextures = textures.ToArray();
            _consumeItemPreviewTextureCache[itemId] = resolvedTextures;
            return resolvedTextures;
        }

        private Texture2D ResolveNpcPreviewTexture(int itemId)
        {
            if (itemId <= 0 || _graphicsDevice == null)
            {
                return null;
            ...
            if (_npcPreviewTextureCache.TryGetValue(itemId, out Texture2D cachedTexture))
            {
                return cachedTexture;
            }

            Texture2D texture = null;
            if (InventoryItemMetadataResolver.TryResolveNpcPreviewCanvas(itemId, out WzCanvasProperty npcCanvas))
            {
                texture = npcCanvas.GetLinkedWzCanvasBitmap()?.ToTexture2DAndDispose(_graphicsDevice);
            }

            _npcPreviewTextureCache[itemId] = texture;
            return texture;
        }
