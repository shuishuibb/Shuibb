...
namespace MapleLib.WzLib.WzProperties
{
    ...
    public class WzPngProperty : WzImageProperty
    {
        ...
        /// <summary>
        /// Maps a stored WzPngFormat back to the SurfaceFormat that re-encodes to it, so an
        /// existing canvas can be rewritten in the format it is already stored in. Returns false
        /// for 0 (a property that has never held an image) and for anything this build cannot
        /// re-encode, in which case the caller falls back to detecting a format.
        /// </summary>
        private static bool TryGetSurfaceFormat(WzPngFormat format, out SurfaceFormat surface, out bool isGrayscale)
        {
            isGrayscale = false;
            switch (format)
            {
                ...
                case WzPngFormat.Format3: surface = SurfaceFormat.Dxt3; isGrayscale = true; return true;
        ...
        internal void CompressPng(Bitmap bmp)
        {
            ...
            ////// Keep the surface format this canvas already has, and only detect one when there
            ////// isn't one yet.
            //
            // Assigning to .PNG is how callers swap the artwork on an EXISTING canvas, and
            // re-detecting silently rewrote the storage format - a BGRA4444 item icon replaced
            // with artwork whose alpha looked 1-bit came back as ARGB1555. That still parses, and
            // the editor previews it correctly because it reads the format it just wrote, but the
            // game reads the canvas with the format it expects and draws garbage.
            //
            // A freshly constructed WzPngProperty leaves 'format' at 0, which is not a valid
            // WzPngFormat, so brand new canvases still get a detected format as before. Callers
            // that deliberately want a different format (ConvertCanvasToBgra32, the resize
            // "force BGRA32" path) go through SetCompressedBytes and are unaffected.
            SurfaceFormat suggested_surfaceFormat;
            bool preserveGrayscale = false;
            bool preservedFormat = TryGetSurfaceFormat(this.format, out suggested_surfaceFormat, out preserveGrayscale);
            if (!preservedFormat)
            {
                // See UnitTest_WzFile/UnitTest_MapleLib.cs/TestImageSurfaceFormatDetection
                suggested_surfaceFormat = ImageFormatDetector.DetermineTextureFormat(pixels, bmp.Width, bmp.Height);
            }
            ...
            (WzPngFormat format, byte[] compressedBytes) = preservedFormat
                ? PngUtility.CompressImageToPngFormat(bmp, suggested_surfaceFormat, preserveGrayscale)
                : PngUtility.CompressImageToPngFormat(bmp, suggested_surfaceFormat);
            ...
            if (listWzUsed)
            {
                // Re-encrypt with the key this image is actually read back with. Hardcoding the
                // GMS key here silently corrupts any file on another encryption (EMS, BMS, a
                // custom one): writing succeeds, but the next read decrypts with the file's own
                // key and the deflate stream is garbage.
                MapleLib.WzLib.Util.WzMutableKey wzKey = this.wzReader?.WzKey ?? ParentImage?.reader?.WzKey;
                ...
                using (MemoryStream memStream = new MemoryStream())
                {
                    using (WzBinaryWriter writer = new WzBinaryWriter(memStream, WzTool.GetIvByMapleVersion(WzMapleVersion.GMS)))
                    {
                        ...
                        for (int i = 0; i < 2; i++)
                        {
                            writer.Write((byte)(compressedImageBytes[i] ^ wzKey[i]));
                        }
                        writer.Write(compressedImageBytes.Length - 2);
                        for (int i = 2; i < compressedImageBytes.Length; i++)
                            writer.Write((byte)(compressedImageBytes[i] ^ wzKey[i - 2]));
