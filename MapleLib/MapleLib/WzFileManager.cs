...
namespace MapleLib {
    ...
    public class WzFileManager : IDisposable {
        ...
        public (string iniFilePath, int fileIndex) GetIniWzIndexInfo(string directoryPath)
        {
            ...
            // Find .ini files in directory. Missing/duplicate/malformed .ini files are treated
            // the same as "directory doesn't exist" (skip this directory) rather than throwing:
            // BuildWzFileList() walks every sibling folder under the detected Data root, and a
            // single incompletely-converted category (common on private-server WZ dumps that
            // only bothered updating some categories to the split 64-bit layout) would otherwise
            // abort opening any WZ file at all. LoadCanvasSection() already relies on this exact
            // (null, -1) sentinel to skip gracefully - this just makes every failure path here
        ...
        public WzFile LoadWzFile(string baseName, WzMapleVersion encVersion) {
            ...
            // Opening a file that is already in memory is a no-op, not an error. Callers already
            // treat null as "nothing to add" (that is what an unresolvable path returns), so
            // reporting it that way lets them skip it quietly. Parsing a second copy and then
            // throwing meant that re-opening an open .wz - or the related-file autoload finding a
            // sibling that was already loaded - raised an exception inside the callers'
            // Parallel.ForEach, which escaped their async void handler as an AggregateException
            // and brought down the whole application with an unhandled-exception dialog.
            ...
            {
                // Lost a race with another thread loading the same file - the check above cannot
                // cover that, since callers load the batch in parallel and a list can legitimately
                // name the same path twice. Same outcome as the pre-check: already loaded.
                wzf.Dispose();
                return null;
            }
            catch
        ...
        public void LoadCanvasSection(string canvasFolder, WzMapleVersion encVersion)
        {
            lock (_canvasSectionLoadLock)
            {
                ...
                // The index is three digits on disk (_canvas_000 ... _canvas_119). Building it
                // as a literal "_0" plus a two-digit number only works up to 99: index 100
        ...
        public void UnloadWzFile(WzFile wzFile, string wzFilePath) {
            // wzFilePath (the WzFile's full on-disk path, e.g. "...\Skill_00000.ms") is NOT the
            // dictionary key: _wzFiles is keyed by the short "baseName" that was passed to
            // LoadWzFile (e.g. "Skill_00000"), and GetWzKey only strips a ".wz" suffix - it does
            // not strip directories, and a ".ms" path doesn't even match that suffix. Recomputing
            // the key from wzFilePath therefore virtually never matches the key actually stored
            // at load time, so this removal used to silently no-op, leaving a stale entry behind
            // that made every later attempt to reload the same file fail with "already loaded".
            // Look the key up by value instead - the same approach UnloadWzImgFile already uses
            // below - since that can't go wrong regardless of what the path looks like.
            string baseName = null;
            foreach (var kvp in _wzFiles)
            {
                if (kvp.Value == wzFile) { baseName = kvp.Key; break; }
            }
            if (baseName != null) {
                // write lock to begin adding to the dictionary
                _readWriteLock.EnterWriteLock();
